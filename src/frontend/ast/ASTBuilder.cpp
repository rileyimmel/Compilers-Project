...
Any ASTBuilder::visitForEachStmt(TIPParser::ForEachStmtContext *ctx) {
    ...
    visit(ctx->expr(1));
    auto list = visitedExpr;

    visit(ctx->statement());
    auto body = visitedStmt;

    visitedStmt = std::make_shared<ASTForEachStmt>(item, list, body);
...
Any ASTBuilder::visitArrExpr(TIPParser::ArrExprContext *ctx) {
    ...
    for (auto element : ctx->expr()) {
        visit(element);
        elements.push_back(visitedExpr);
    }

    visitedExpr = std::make_shared<ASTArrExpr>(elements);
...
Any ASTBuilder::visitArrElemRefExpr(TIPParser::ArrElemRefExprContext *ctx) {
    visit(ctx->expr(0));
    auto ptr = visitedExpr;

    visit(ctx->expr(1));
    auto index = visitedExpr;

    visitedExpr = std::make_shared<ASTArrElemRefExpr>(ptr, index);
